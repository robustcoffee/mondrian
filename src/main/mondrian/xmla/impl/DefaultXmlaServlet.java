...
public abstract class DefaultXmlaServlet extends XmlaServlet {
   ...
   protected void unmarshallSoapMessage(HttpServletRequest request, Element[] requestSoapParts) throws XmlaException {
      try {
         ...
         NodeList sub = childs[0].getElementsByTagName("Discover");
         NodeList sub2 = childs[0].getChildNodes();
         for (int i = 0; i < sub2.getLength(); i++) {
            Node item = sub2.item(i);
            item.getNodeName();
            System.out.println(item.getFirstChild());
            System.out.println(item.getNodeName());

            if (item.getNodeName().equals("Discover")) {
               if (item.hasChildNodes()) {
                  String tmp1 = item.getFirstChild().getTextContent();
                  Node node1 = item.getFirstChild().getFirstChild();
                  String name = item.getFirstChild().getNodeName();
                  String value = item.getFirstChild().getNodeValue();
                  if (name.equals("RequestType") && tmp1.equals("DISCOVER_SCHEMA_ROWSETS")) {
                     isDiscoverRowSet = true;
    ...
    * EndSession mustUnderstand=1 in the
    * "urn:schemas-microsoft-com:xml-analysis" namespace Header elements and a
    * NamespaceCompatibility mustUnderstand=0 in the
    * "http://schemas.microsoft.com/analysisservices/2003/xmla" namespace. Here
    * we handle only the session Header elements.
    * 
    * <p>
   ...
   protected void handleSoapHeader(HttpServletResponse response, Element[] requestSoapParts, byte[][] responseSoapParts, Map<String, Object> context) throws XmlaException {
      try {
         ...
         for (int i = 0; i < nlen; i++) {
            ...
            boolean mustUnderstandValue = attr != null && attr.getValue() != null && attr.getValue().equals("1");
            ...
            if (localName.equals(XMLA_BEGIN_SESSION)) {
               ...
               if (sessionInfo != null) {
                  ...
                  context.put(CONTEXT_XMLA_USERNAME, "ads");
                  context.put(CONTEXT_XMLA_PASSWORD, "ads");
               }
   ...
   protected void marshallSoapMessage(HttpServletResponse response, byte[][] responseSoapParts, Enumeration.ResponseMimeType responseMimeType) throws XmlaException {
      try {
       ...
       //  String encoding = "UTF-16";
         /*
          * Since we just reset response, encoding and content-type were reset
          * too
          */
   ...
   private SessionInfo getSessionInfo(String sessionId) {
      ...
      if (sessionInfo == null) {
         ...
         return saveSessionInfo("ads", "", sessionId);
      } else {
