...
public class JdbcDialectImpl implements Dialect {
	 ...
	 * 
	 * <p>
	 * To prevent connection leaks, this constructor does not hold a reference
	 * to the connection after the call returns. It makes a copy of everything
	 * useful during the call. Derived classes must do the same.
	 * </p>
	 * 
	 * @param connection
	 *            Connection
	...
	protected Set<List<Integer>> deduceSupportedResultSetStyles(
		...
		for (int type : RESULT_SET_TYPE_VALUES) {
			for (int concurrency : CONCURRENCY_VALUES) {
				try {
					if (databaseMetaData.supportsResultSetConcurrency(type,
							concurrency)) {
						String driverName = databaseMetaData.getDriverName();
						if (type != ResultSet.TYPE_FORWARD_ONLY
								&& driverName
										.equals("JDBC-ODBC Bridge (odbcjt32.dll)")) {
						...
						supports.add(new ArrayList<Integer>(Arrays.asList(type,
								concurrency)));
					}
				} catch (SQLException e) {
	...
	public void quoteBooleanLiteral(StringBuilder buf, String value) {
		...
		if (!value.equalsIgnoreCase("TRUE")
			...
			throw new NumberFormatException("Illegal BOOLEAN literal:  "
					+ value);
		}
		buf.append(value);
	}
	...
	public void quoteTimestampLiteral(StringBuilder buf, String value) {
		...
		try {
			...
			throw new NumberFormatException("Illegal TIMESTAMP literal:  "
					+ value);
		}
	 ...
	 * clause, specified by the caller of this method, appropriate to the
	 * dialect of SQL.
	 * 
	 * @param columnNames
	 *            Column names
	 ...
	 * 
	 * <p>
	 * If NULL values are present, we use a CAST to ensure that they have the
	 * same type as other columns:
	 * 
	 * <blockquote><code>SELECT * FROM
	...
	protected String generateOrderByNulls(String expr, boolean ascending,
		...
		if (collateNullsLast) {
			...
			if (ascending) {
				return "CASE WHEN " + expr + " IS NULL THEN 0 ELSE 1 END, "
						+ expr + " ASC";
			} else {
				return "CASE WHEN " + expr + " IS NULL THEN 0 ELSE 1 END, "
						+ expr + " DESC";
	...
	public boolean supportsResultSetConcurrency(int type, int concurrency) {
		return supportedResultSetTypes.contains(Arrays
	...
	void logTypeInfo(ResultSetMetaData metaData, int columnIndex,
			SqlStatement.Type internalType) throws SQLException {
		if (LOGGER.isDebugEnabled()) {
			final int columnType = metaData.getColumnType(columnIndex + 1);
			final int precision = metaData.getPrecision(columnIndex + 1);
			final int scale = metaData.getScale(columnIndex + 1);
			final String columnName = metaData.getColumnName(columnIndex + 1);
			LOGGER.debug("JdbcDialectImpl.getType " + "Dialect- "
					+ this.getDatabaseProduct() + ", Column-" + columnName
					+ " is of internal type " + internalType
					+ ". JDBC type was " + columnType + ".  Column precision="
					+ precision + ".  Column scale=" + scale);
		}
	}
	...
	protected List<StatisticsProvider> computeStatisticsProviders() {
		...
		if (names == null) {
			return Collections
					.<StatisticsProvider> singletonList(new SqlStatisticsProvider());
		}
		final List<StatisticsProvider> providerList = new ArrayList<StatisticsProvider>();
		for (String name : names) {
			try {
				StatisticsProvider provider = ClassResolver.INSTANCE
						.instantiateSafe(name);
				providerList.add(provider);
			} catch (Exception e) {
				LOGGER.info("Error instantiating statistics provider (class="
						+ name + ")", e);
			}
		}
		return providerList;
	...
	private List<String> getStatisticsProviderNames() {
		...
		final String path = MondrianProperties.instance().StatisticsProviders
				.getPath() + "." + getDatabaseProduct().name();
	...
	public static DatabaseProduct getProduct(String productName,
		...
		} else if (upperProductName.equals("ADS")) {
			return DatabaseProduct.ADS;
	...
	/**
	 * Helper method to determine if a connection would work with a given
	 * database product. This can be used to differenciate between databases
	 * which use the same driver as others.
	 * 
	 * <p>
	 * It will first try to use
	 * {@link DatabaseMetaData#getDatabaseProductName()} and match the name of
	 * {@link DatabaseProduct} passed as an argument.
	 * 
	 * <p>
	 * If that fails, it will try to execute <code>select version();</code> and
	 * obtains some information directly from the server.
	 * 
	 * @param databaseProduct
	 *            Database product instance
	 * @param connection
	 *            SQL connection
	 * @return true if a match was found. false otherwise.
	 */
	protected static boolean isDatabase(DatabaseProduct databaseProduct,
			...
			if (resultSet.next()) {
				...
				if (version != null) {
					if (version.toLowerCase().contains(dbProduct)) {
						LOGGER.info("Using " + databaseProduct.name()
								+ " dialect");
						return true;
					}
				}
